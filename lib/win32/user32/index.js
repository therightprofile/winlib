...
module.exports = {
  ...
  SendMessage: async function (hWnd, Msg, wParam, lParam) {
    ...
    SendMessageA: async function (hWnd, Msg, wParam, lParam) {
    hWnd = hWnd || '0';
    Msg = Msg || '0';
    wParam = wParam || '0';
    lParam = lParam || '0';
    let cmdStr = `${__dirname}\\user32.exe SendMessageA "${hWnd}" "${Msg}" "${wParam}" "${lParam}"`;
    if (this.isDebug === true) console.log(`called SendMessage: ${cmdStr}`);
    let eObj = await asyncExec(cmdStr);
    return filterOutput(eObj.data);
  },
  ...
  GetWindowThreadProcessId: async function (hWnd, lpdwProcessId ) {
    hWnd = hWnd || '0';
    lpdwProcessId = lpdwProcessId || null;
    let cmdStr = `${__dirname}\\user32.exe GetWindowThreadProcessId "${hWnd}" "${lpdwProcessId}"`;
